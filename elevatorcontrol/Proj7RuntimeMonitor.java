...
public class Proj7RuntimeMonitor extends RuntimeMonitor {
    ...
    ConditionalStopwatch stopwatch = new ConditionalStopwatch();
    boolean hasMoved = false;
    boolean wasOverweight = false;
    boolean weightWasChanged = false;
    int overWeightCount = 0;
    int currentWeight = 0;
    int numOpens = 0;
    int numWastedOpens = 0;
    ...
    protected String[] summarize() {
        ...
        arr[2] = "Wasted Openings = " + numWastedOpens;
        arr[3] = "Door reversal time = " + doorReversalTime;
        return arr;
    }
    ...
    private void doorReopening(Hallway hallway) {
        ...
        if(doorState.anyDoorMotorOpening(Hallway.FRONT) || doorState.anyDoorMotorOpening(Hallway.BACK)) {
            stopwatch.commit();
            doorReversalTime = SimTime.add(doorReversalTime, stopwatch.getAccumulatedTime());
            stopwatch.reset();            
        }
    }
    ...
    private void doorClosed(Hallway hallway) {
        ...
        if (!doorState.anyDoorOpen()) {
            if (weightWasChanged) {
                message("Passenger Entered/Exited");
                numWastedOpens--;
    ...
    private void doorOpened(Hallway hallway) {
        ...
        if(doorState.allDoorsCompletelyOpen(Hallway.FRONT) || doorState.allDoorsCompletelyOpen(Hallway.BACK)) {
            numWastedOpens++;
            numOpens++;
        }
    }
